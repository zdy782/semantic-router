...
/*
#include <stdlib.h>
extern void* candle_sequence_model_open(const char* options, char** error);
extern char* candle_sequence_model_predict(void* model, const char* text);
extern char* candle_sequence_model_predict_windows(void* model, const char* text, size_t size, size_t overlap);
extern void candle_sequence_model_close(void* model);
extern void candle_sequence_model_free(char* value);
*/
import "C"
...
func OpenSequenceModel(options SequenceModelOptions) (*SequenceModel, error) {
	...
	handle := C.candle_sequence_model_open(input, &nativeError)
	if nativeError != nil {
		defer C.candle_sequence_model_free(nativeError)
	}
	if handle == nil {
		if nativeError == nil {
...
func (m *SequenceModel) Classify(text string) ([]float32, error) {
	...
	defer C.free(unsafe.Pointer(input))
	response := C.candle_sequence_model_predict(m.handle, input)
	if response == nil {
		return nil, fmt.Errorf("sequence model returned no response")
	}
	defer C.candle_sequence_model_free(response)
	var result struct {
...
func (m *SequenceModel) ClassifyWindows(text string, options SequenceWindowOptions) ([]SequenceWindowScores, error) {
	...
	input := C.CString(text)
	defer C.free(unsafe.Pointer(input))
	response := C.candle_sequence_model_predict_windows(m.handle, input, C.size_t(options.Size), C.size_t(options.Overlap))
	if response == nil {
...
func (m *SequenceModel) Close() error {
	...
	defer m.mu.Unlock()
	if m.handle != nil {
		C.candle_sequence_model_close(m.handle)
		m.handle = nil
	}
	return nil
}
