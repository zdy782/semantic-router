...
fn load(options: Options) -> Result<SequenceModel, String> {
    ...
    let model =
        ...
        .map_err(|e| e.to_string())?;
    if model.get_num_classes() != options.labels.len() {
        return Err("loaded classifier label count differs from its contract".into());
    }
    Ok(SequenceModel {
        model,
        tokenizer,
...
#[no_mangle]
pub unsafe extern "C" fn candle_sequence_model_open(
...
#[no_mangle]
pub unsafe extern "C" fn candle_sequence_model_predict(
    handle: *mut c_void,
    text: *const c_char,
) -> *mut c_char {
    let outcome = catch_unwind(AssertUnwindSafe(|| -> Result<Vec<f32>, String> {
        ...
        let (_, _, scores) = model
            .model
            .classify_text_with_activation(text, model.multi_label)
            .map_err(|e| e.to_string())?;
        Ok(scores)
    }))
...
#[no_mangle]
pub unsafe extern "C" fn candle_sequence_model_free(value: *mut c_char) {
    if !value.is_null() {
...
mod tests {
    ...
    fn sequence_ffi_returns_owned_errors_for_invalid_arguments() {
        ...
        unsafe {
            assert!(candle_sequence_model_open(std::ptr::null(), &mut error).is_null());
            assert!(!error.is_null());
            candle_sequence_model_free(error);
            let result = candle_sequence_model_predict(std::ptr::null_mut(), std::ptr::null());
            assert!(CStr::from_ptr(result).to_str().unwrap().contains("error"));
            candle_sequence_model_free(result);
            candle_sequence_model_close(std::ptr::null_mut());
        }
    }
}
